...
- **FR-2.3** Idle time shall be counted only within the daily working window (between session start and session end); time outside this window is untracked. Idle time that starts exactly at the session start or session end boundaries shall not be counted to cleanly separate work periods from pre/post-session computer use.
